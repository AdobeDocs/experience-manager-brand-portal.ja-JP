...
フォルダーのアセットに対するアクセス権は、その親フォルダーに対するアクセス権に依存します。子フォルダーの設定には関係ありません。AEMの [ACL](https://experienceleague.adobe.com/ja/docs/experience-manager-65/content/security/security) によって制御され、子フォルダーは親フォルダーから ACL を継承します。 例えば、フォルダー A にフォルダー B が含まれ、その中にフォルダー C が含まれているとします。次に、フォルダー A に対するアクセス権を持つユーザーグループ（またはユーザー）は、フォルダー B とフォルダー C に対しても同じアクセス権を持ちます。フォルダー B （A の子フォルダー）は ACL を継承し、フォルダー C （B の子フォルダー）は ACL を継承します。
